...
{
    _getTime(time, _logger_config.clockType);
}
...
int logger_writeListToCSV(const char *fileName, int *exportList, int exportListCount, logger_tagDef_t *logDef, int logDefCount)
{
    ...
    for (int j = 0; j < _logger_config.listCount; j++)
    {
        ...
        if (_logger_logEntryList[j][0].time_stamp.tv_sec > 0 && _logger_logEntryList[j][0].time_stamp.tv_sec < startTime)
        {
    ...
    for (int j = 0; j < _logger_config.listCount; j++)
    {
        ...
        for (int i = 0; i < _logger_nextEntry[j]; i++)
        {
            ...
            fprintf(pFile, "%s,%lu,%d.%s%ld\n", info,
                    lEntr->id,
                    (int)(lEntr->time_stamp.tv_sec - startTime),
                    zeroString,
                    lEntr->time_stamp.tv_nsec);
        }
    }
